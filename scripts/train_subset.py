...
def step_validate(
    metrics: dict, threshold: float, mlflow_uri: str, run_id: str
) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--src", type=Path, default=DEFAULT_SRC, help="Raw VISION dataset root"
    )
    p.add_argument(
        "--dst",
        type=Path,
    ...
    p.add_argument(
        "--model-name", default=DEFAULT_MODEL, help="Registered model name in MLflow"
    )
    p.add_argument(
        "--resume-from",
        type=Path,
...
def main() -> None:
    ...
    run_id, metrics = step_train(
        yaml_path, args.epochs, args.mlflow_uri, args.resume_from
    )
